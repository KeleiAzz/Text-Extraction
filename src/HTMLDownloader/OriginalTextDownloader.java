...
public class OriginalTextDownloader
{
	...
	public static void main(String args[])
	{
		...
		PhraseReader phraseReader = new PhraseReader("/Users/keleigong/Dropbox/work/textExtraction/phrase.xlsx");
		phraseDictionary = phraseReader.ReadPhraseExcel();
		System.out.println("there r "+phraseDictionary.size()+" phrase dictionary items");

		//load stopwords list
		UrlListReader stopWordReader = new UrlListReader("/Users/keleigong/Dropbox/work/textExtraction/stopwords.xlsx");
		...
		try
		{
			companyNames = FileFolderIterator.readCompanyNames("/Users/keleigong/Dropbox/work/textExtraction/b.company_url");
		...
		for(int j=0;j<companyNames.size();j++)//每次循环读一个company的所有url
		{
			...
			FileOperator.newFolder("/Users/keleigong/Dropbox/work/textExtraction/pdfdownloader/"+companyNames.get(j));

			//Read URL list
			UrlListReader urlReader = new UrlListReader("/Users/keleigong/Dropbox/work/textExtraction/b.company_url/"+companyNames.get(j)+".xlsx");
			...
			for(int i = 0; i<=urlArrayList.size()-1;i++)
			{
				...
				else//pdf URL
				{
					pdfCounter++;
					System.out.println(urlArrayList.get(i));
					boolean res = PdfDownloader.downloadFromUrl(urlArrayList.get(i).replace("http//", "http://"),"/Users/keleigong/Dropbox/work/textExtraction/pdfdownloader/"+companyNames.get(j)+"/",companyNames.get(j)+pdfCounter+".pdf");
					...
					else//如果能下载,读取文件内容并打印
					{
						...
						AllCompanyContent+=urlArrayList.get(i)+"\r\n"+PdfReaderOriginalText.readFileOfPDF("/Users/keleigong/Dropbox/work/textExtraction/pdfdownloader/"+companyNames.get(j)+"/"+companyNames.get(j)+pdfCounter+".pdf")+"\r\n==========\r\n";
					}
				}
			...
			CSVWriter.Write("/Users/keleigong/Dropbox/work/textExtraction/deadlink.csv",deadLinkString);


			//write csv for current company
			CSVWriter.Write("/Users/keleigong/Dropbox/work/textExtraction/c.company_text/company_csv/"+companyNames.get(j)+".txt",AllCompanyContent);
